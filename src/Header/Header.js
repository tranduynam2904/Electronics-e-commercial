...
export default function Header() {
  ...
  }, [nav, show, setNav, setShow, setUser]);
  useEffect(() => {
    handle_resize();
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (
        menuRef.current &&
        window.pageYOffset >= menuRef.current.offsetTop + window.innerHeight / 4
      ) {
        setMenuHeight("120px");
      } else {
        setMenuHeight("100px");
    ...
    <>
      <div className="header">
        <div
          ...
          className="handle_logo"
        >
          {nav && (
            <buton onClick={handle_toggle} className="handle_toggle">
              <img src={toggle} alt="Toggle Menu"></img>
            </buton>
          )}
          <Link
            className="logo_link"
          ...
          <div className="search_area">
            <p className="handle_search_input">
              <input
                type="text"
                  ...
                  {SearchResultLatest.map((item) => {
                    return (
                      <li>
                        <Link
                          onClick={() =>
                            (window.location.href = `/product/latest/${item.id}`)
                          }
                          key={item.id}
                        >
                  ...
                  {SearchResult.map((item) => {
                    return (
                      <li>
                        <Link
                          onClick={() =>
                            (window.location.href = `/product/${item.id1}`)
                          }
                          key={item.id1}
                        >
                          <img src={item.image} alt={item.name}></img>
          ...
          <div className="handle_contact">
            ...
            <div className="handle_user">
              {!nav && (
                <Link style={{ color: "#000" }} onClick={handleSignIn}>
                  <HiOutlineUser
                    className="user_icon"
                    style={{
                      cursor: "pointer",
              ...
              {!nav && (
                ...
                >
                  ...
                  <div className="handle_divcartlenght">
                    <span className="handle_cartlenght">
                      {cart.length + cartLatestProduct.length}
                    </span>
                  </div>
                </Link>
              )}
        ...
        <nav>
          <ul className="handle_menu" style={{ listStyle: "none" }}>
            <div className="handle_categories">
              {show && (
                ...
                >
                  <FiMenu className="menu_icon" /> SHOP BY CATEGORIES
                </li>
              )}
            </div>
            {!nav && (
          ...
          <div className="handle_cate_li" style={{ display: "flex" }}>
            ...
            <div className="handle_path">
              <Routes>
                ...
                <Route
                  path={"/product/:id1"}
                  element={<ProductDetailProduct />}
                ></Route>
                <Route
                  path={"/product/latest/:id"}
                  element={<ProductDetailProductLatest />}
                ></Route>
              </Routes>
            </div>
          </div>
          {user && (
            <div>
              {nav && (
                <div
                  className={`handle_containdropdown`}
                  style={{ display: "flex", justifyContent: "center" }}
                >
                  <ul className="cate_li">
                    <li
                      onClick={handle_catedropdown}
                      className="menu_categories"
                      style={{
                        whiteSpace: "nowrap",
                        fontSize: "0.8rem",
                    ...
                    <div
                      className="handle_tabmenucategories"
                      style={{ overflow: "hidden", transition: "1s" }}
                    >
                      <div
                        className={`handle_tabmenu ${
                          categories ? "active1" : ""
                        }`}
                      >
                        <Tabmenu />
                    ...
                    <div
                      className={`handle_bestselling ${
                        categories ? "active2" : ""
                      }`}
                    >
                      <p
                        style={{ cursor: "pointer" }}
                        onClick={handle_bestselling_click}
                        className="title"
                      >
                        <FiMenu
                          style={{ fontSize: "1.3rem" }}
                        ...
                        >
                          {product &&
                            product.map((item, index) => (
                              <div
                                index={index}
                                className="handle_cardbestselling"
                              >
                                <div className="img">
                                  <Link
                                    // to={`/product/${item.id1}`}
                                ...
                                <div className="handle_title">
                                  <div>
                                    <h1 style={{ color: "#0364BE" }}>
                                      {item.name}
                                    </h1>
                                    <p>${item.price}</p>
                                    <div className="button_addtocart">
                                      <button onClick={() => addCart(item.id1)}>
                                        ADD TO CART
                                      </button>
                                    </div>
                                  </div>
                    ...
                    >
                      <p
                        onClick={handle_click_clientsays}
                        className="title"
                      ...
                      <div
                        className={`handle_client ${!client ? "active1" : ""}`}
                      >
                        <Slider className="client" {...settings}>
                          <div className="padding_client">
                            <div className="handle_avatar_respond">
                              <img src={ava1} alt="Hana Christopher"></img>
                            </div>
                            <p>
                              Lorem Ipsum is simply dummy text of the printing
                              and typesetting industry.
                            </p>
                            <Link className="client_name">
                              HANA CHIRSTOPHER
                            </Link>
                          </div>
                          <div className="padding_client">
                            <div className="handle_avatar_respond">
                              <img src={ava2} alt="Trinity Irish"></img>
                            </div>
                            <p>
                          ...
                          <div className="padding_client">
                            <div className="handle_avatar_respond">
                              <img src={ava3} alt="John Benson"></img>
                            </div>
                            <p>
                          ...
                          <div className="padding_client">
                            <div className="handle_avatar_respond">
                              <img src={ava4} alt="Henry Lau"></img>
                            </div>
                            <p>
